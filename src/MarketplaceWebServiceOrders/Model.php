...
abstract class MarketplaceWebServiceOrders_Model {
    ...
    protected function _toQueryParameterArray($prefix) {
        ...
        foreach($this->_fields as $fieldName => $fieldAttrs) {
            ...
            $currentArr = $this->_toQueryParameterArray2($newPrefix, $fieldType, $fieldValue, $fieldAttrs);
            $arr = array_merge($arr, $currentArr);
        }
        return $arr;
    }
    
    private function _toQueryParameterArray2($prefix, $fieldType, $fieldValue, $fieldAttrs) {
        $arr = array();
        if(is_array($fieldType)) {
            ...
            for($i = 1; $i <= $count; $i++) {
                ...
                $arr = array_merge($arr, $this->_toQueryParameterArray2($indexedPrefix, $memberType, $fieldValue[$i - 1], null));
            }
    ...
    protected function _toXMLFragment() {
        ...
        foreach($this->_fields as $fieldName => $field) {
            ...
            if($fieldValue !== null && $field['FieldType'] !== 'MarketplaceWebServiceOrders_Model_ResponseHeaderMetadata') {
                $fieldType = $field['FieldType'];
                if(is_array($fieldType)) {
                    if($fieldType[0] === 'object') {
     ...
     */
    private function _escapeXML($xml) {
        $from = ['&', '<', '>', "'", '"'];
        $to = ['&amp;', '&lt;', '&gt;', '&#039;', '&quot;'];
        return str_replace($from, $to, $xml);
    }
    ...
    private function _isComplexType($fieldType) {
        return 0 === strpos($fieldType, 'MarketplaceWebServiceOrders_');
    }
